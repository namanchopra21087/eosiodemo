...
void ordercontrct::addproduct(const string &title, const string &description, const string &url, const float &price)
{
...
void ordercontrct::addorder( uint16_t userid,
                         const vector<uint8_t> & items,
                         const string & status )
{
    addorder_impl(userid, items, status, false);
}

void ordercontrct::updateorder(uint16_t id, uint16_t userid, const vector<uint8_t> &items, const string &status)
{
